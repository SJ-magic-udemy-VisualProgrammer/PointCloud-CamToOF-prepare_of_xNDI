...
//
bool ofxNDIreceive::ReceiveImage(unsigned char *pixels, unsigned int &width, unsigned int &height, bool bInvert)
{
...
// Receive RGBA image pixels to a buffer
//
bool ofxNDIreceive::ReceiveImage_Find1stVideo(unsigned char *pixels, unsigned int &width, unsigned int &height, bool bInvert)
{
	/********************
	********************/
	...
	if(b_1st){
		printf("> RECEIVEIMAGE_TYPE == RECEIVEIMAGE_FIRST_VIDEO\n");
		fflush(stdout);
		b_1st = false;
	}
	...
	********************/
	...
	for(int i = 0; i < 16; i++){
		...
		if (!m_metadataString.empty()) m_metadataString.clear();
		...
		switch (NDI_frame_type) {
			...
			case NDIlib_frame_type_video:
				if( video_frame.p_data ){
					...
					}else if( video_frame.p_data && (uint8_t*)pixels ){ // Otherwise sizes are current - copy the received frame data to the local buffer
						switch (video_frame.FourCC) {
							// Note : If the receiver is set up to prefer BGRA or RGBA format,
							// the slower YUV422_to_RGBA conversion function here is not used.
							case NDIlib_FourCC_type_UYVY: // YCbCr color space
...
// Receive RGBA image pixels to a buffer
//
bool ofxNDIreceive::ReceiveImage_FindLastVideo(unsigned char *pixels, unsigned int &width, unsigned int &height, bool bInvert)
{
	/********************
	********************/
	static bool b_1st = true;
	...
	if(b_1st){
		printf("> RECEIVEIMAGE_TYPE == RECEIVEIMAGE_LATEST_VIDEO\n");
		fflush(stdout);
		b_1st = false;
	}
	...
	********************/
	...
	for(int i = 0; i < 16; i++){
		...
		NDIlib_video_frame_v2_t video_frame_local;
		
		m_FrameType = NDIlib_frame_type_none;
		
		// Vers 4.5
		// NDI_frame_type = p_NDILib->recv_capture_v2(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 0);
		NDI_frame_type = p_NDILib->recv_capture_v3(pNDI_recv, &video_frame_local, &audio_frame, &metadata_frame, 0);
		...
		if(NDI_frame_type == NDIlib_frame_type_none) break;
		
		switch (NDI_frame_type) {
			// for loopから抜けるので ↑ へ持っていった
			// No data received or the connection lost
			// case NDIlib_frame_type_none:
			//	return false;
			...
			case NDIlib_frame_type_video:
				if(b_got_video)	FreeVideoData();
				
				if( video_frame_local.p_data ){
					// The caller can check whether a frame has been received
					bReceiverConnected = true;
					
					if (m_Width != (unsigned int)video_frame_local.xres || m_Height != (unsigned int)video_frame_local.yres) {
						m_Width = (unsigned int)video_frame_local.xres;
						m_Height = (unsigned int)video_frame_local.yres;
						
						width = m_Width;
						height = m_Height;
						
						p_NDILib->recv_free_video_v2(pNDI_recv, &video_frame_local); // 注) ここで解放しているのは、"video_frame_local"
						
						return true;
						
					}else{
						video_frame = video_frame_local;
						b_got_video = true;
						continue;
					}
				}else {
	...
	if( b_got_video ){
		if( video_frame.p_data && (uint8_t*)pixels ){ // Otherwise sizes are current - copy the received frame data to the local buffer
			switch (video_frame.FourCC) {
				// Note : If the receiver is set up to prefer BGRA or RGBA format,
				// the slower YUV422_to_RGBA conversion function here is not used.
				case NDIlib_FourCC_type_UYVY: // YCbCr color space
...
bool ofxNDIreceive::ReceiveImage(unsigned int &width, unsigned int &height)
{
	...
	********************/
	...
	if(b_1st){
		printf("> RECEIVEIMAGE_TYPE == RECEIVEIMAGE_ORIGINAL\n");
		fflush(stdout);
		b_1st = false;
	}
	...
	********************/
	...
	if (pNDI_recv) {
		...
		// Vers 4.5
		// NDI_frame_type = p_NDILib->recv_capture_v3(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 0);
		NDI_frame_type = p_NDILib->recv_capture_v3(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 0); // sj
		// printf("type = %d\n", (int)NDI_frame_type); // sj
...
/******************************
******************************/
bool ofxNDIreceive::ReceiveImage_Find1stVideo(unsigned int &width, unsigned int &height, FILE* fp_log)
{
	/********************
	********************/
	static bool b_1st = true;
	...
	********************/
	...
	int c_status_change	= 0;
	int c_source_change	= 0;
	int c_metadata		= 0;
	int c_audio			= 0;
	int c_video			= 0;
	
	static int64_t prev_timestamp = -1;
		...
		switch (NDI_frame_type) {
			...
			case NDIlib_frame_type_none:
				if(fp_log){
					...
					int elapsed = static_cast<int>( std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start).count() );
					fprintf( fp_log, "%d,%d,%d,%d,%d,%d,%d,1\n", c_status_change, c_source_change, c_metadata, c_audio, c_video, c_status_change + c_source_change + c_metadata + c_audio + c_video, elapsed );
				}
				return false;
			...
			case NDIlib_frame_type_video :
				if (video_frame.p_data) {
					...
					if(fp_log){
						...
						if( prev_timestamp != -1 ){
							int64_t delta = video_frame.timestamp - prev_timestamp;
							double delta_ms = delta * 100e-6;
							fprintf( fp_log, "%d,%d,%d,%d,%d,%d,%d,0,%.3f\n", c_status_change, c_source_change, c_metadata, c_audio, c_video, c_status_change + c_source_change + c_metadata + c_audio + c_video, elapsed, delta_ms );
						}
						
						prev_timestamp = video_frame.timestamp;
...
/******************************
******************************/
bool ofxNDIreceive::ReceiveImage_FindLastVideo(unsigned int &width, unsigned int &height, FILE* fp_log)
{
	/********************
	********************/
	static bool b_1st = true;
	...
	********************/
	...
	// while(true){
	for(int i = 0; i < 16; i++){ // 一応、この場合も安全弁として回数制限を設けておこう
		NDIlib_frame_type_e NDI_frame_type;
		NDIlib_metadata_frame_t metadata_frame;
		// NDIlib_audio_frame_v2_t audio_frame;
		// Vers 4.5
		...
		// Vers 4.5
		NDI_frame_type = p_NDILib->recv_capture_v3(pNDI_recv, &video_frame_local, &audio_frame, &metadata_frame, 0); // sj
		// printf("type = %d\n", (int)NDI_frame_type); // sj
